...
proc ctrlsys_set_parameterized_sensor_ports {core} {
    set dependency {(spirit:decode(id('MODELPARAM_VALUE.NUM_SENSORS')) - 1)}

    foreach port_name {spi_miso} {
        set port [ipx::get_ports -quiet $port_name -of_objects $core]
        if {$port eq ""} {
            error "Missing parameterized SPI port '$port_name'"
        }

        set_property is_vector true $port
        set_property size_left 2 $port
        set_property size_left_resolve_type dependent $port
        set_property size_left_dependency $dependency $port
        set_property size_right 0 $port
        set_property size_right_resolve_type immediate $port
        set_property size_right_dependency {} $port
    }
}
...
proc ctrlsys_verify_parameters {core} {
    set expected [lsort {BUFFER_SIZE NUM_SENSORS}]
    set actual {}
    ...
    if {$actual ne $expected} {
        error "Expected customization parameters $expected, found $actual"
    }

    foreach name $expected {
        set user_parameter [ipx::get_user_parameters $name -of_objects $core]
        set model_parameter [ipx::get_hdl_parameters $name -of_objects $core]
        if {$user_parameter eq "" || $model_parameter eq ""} {
            error "Parameter '$name' is not connected to an HDL model parameter"
        }
        set_property enablement_resolve_type immediate $user_parameter
        set_property enablement_value true $user_parameter
    }
}
...
proc update_MODELPARAM_VALUE.BUFFER_SIZE {MODELPARAM_VALUE.BUFFER_SIZE PARAM_VALUE.BUFFER_SIZE} {
    ...
    close $stream
    puts "Wrote XGUI with NUM_SENSORS and BUFFER_SIZE: $xgui_path"
}
...
proc ctrlsys_build_package {repo_root opts_list} {
    ...
    ctrlsys_set_parameterized_sensor_ports $core
