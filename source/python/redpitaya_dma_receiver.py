...
def print_packet_trailer(data: bytes, frame_words: int) -> None:
    ...
    valid = (
        trailer.magic == PACKET_MAGIC
        and trailer.trailer_bytes == PACKET_TRAILER_BYTES
        and trailer.packet_bytes == PACKET_BYTES
        and trailer.valid_data_bytes <= trailer.trailer_offset
        and trailer.trailer_offset == PACKET_TRAILER_OFFSET
        and trailer.intan_frame_count <= trailer.max_intan_frame_count
    )
