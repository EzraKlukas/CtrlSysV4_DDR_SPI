...
localparam int CHUNK_BYTES = CHUNK_BITS / 8;
...
begin
    ...
    trailer.intan_frame_count = intan_count;
            ...
            case (state)
                WAIT_INPUT: begin
                    if (!packet_active) begin
                        if (Intan_frame_done && packet_ready) begin
                            packet_active <= 1'b1;
                            packet_intan_frames <= 1'b1;
                            packet_word_index <= '0;
                            chunk_count <= '0;
                            word_assembly <= '0;
                            intan_shift <= intan_to_stream_order(Intan_frame_in);
                            intan_chunks_remaining <= INTAN_FRAME_CHUNKS;
                            state <= STREAM_INTAN;
                            ...
                            icm_chunks_remaining <= ICM_FRAME_CHUNKS;
                        ...
                        state <= STREAM_INTAN;
                                 ...
                                 packet_intan_frames < MAX_INTAN_FRAMES) begin
                        intan_shift <= intan_to_stream_order(Intan_frame_in);
                        intan_chunks_remaining <= INTAN_FRAME_CHUNKS;
                ...
                STREAM_INTAN: begin
                    ...
                    if (output_slot_available) begin
                        ...
                        if (chunk_count == CHUNKS_PER_WORD - 1) begin
                ...
                PAD_PACKET: begin
                    ...
                    if (output_slot_available) begin
                        ...
                        end else if (packet_word_index < PACKET_DATA_WORDS) begin
                            word_data <= '0;
                            word_valid <= 1'b1;
                            packet_word_index <= packet_word_index + 1'b1;
                        end else begin
                            trailer_words_remaining <= PACKET_TRAILER_WORDS;
