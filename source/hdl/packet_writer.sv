...
module packet_writer #(
    ...
    localparam int TRAILER_BYTE_INDEX_WIDTH = (PACKET_TRAILER_BYTES > 1) ? $clog2(
    ...
    localparam int INTAN_FRAME_COUNT_WIDTH = (MAX_INTAN_FRAMES > 1) ? $clog2(
        MAX_INTAN_FRAMES + 1
    ) : 1;
    ...
    function automatic logic [7:0] intan_frame_byte(
        input Intan_frame_t frame, input logic [INTAN_BYTE_INDEX_WIDTH-1:0] byte_index);
        int unsigned msb;
        begin
    ...
    function automatic logic [7:0] icm_frame_byte(
        input ICM_frame_t frame, input logic [ICM_BYTE_INDEX_WIDTH-1:0] byte_index);
        int unsigned msb;
        begin
    ...
    function automatic logic [7:0] u32_be_byte(input logic [31:0] value,
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            case ({
                intan_fifo_write, intan_fifo_read
            })
            ...
            case ({
                intan_frame_write_done, intan_frame_read_done
            })
            ...
            if (state == IDLE && packet_ready && icm_pending && !word_valid) begin
                ...
                if (complete_intan_frames == 0) state <= STREAM_ICM;
                else state <= STREAM_INTAN;
