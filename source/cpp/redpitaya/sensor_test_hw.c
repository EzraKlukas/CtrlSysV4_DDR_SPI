...
#define COMMAND_CLEAR                                                          \
  (SENSOR_TEST_COMMAND_CLEAR_ERROR | SENSOR_TEST_COMMAND_RESET_SAMPLE_COUNT |  \
...
#define SPI_SR_ERROR                                                           \
  ((1u << 4) | (1u << 6) | (1u << 7) | (1u << 8) | (1u << 9) | (1u << 10))
...
#define ICM_INTAN_PACKET_TRAILER_OFFSET SENSOR_TEST_PACKET_TRAILER_OFFSET
...
static volatile uint32_t *map_registers(int mem_fd, off_t address) {
  void *mapping =
      mmap(NULL, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, mem_fd, address);
  if (mapping == MAP_FAILED) {
    fprintf(stderr, "mmap(0x%08jx): %s\n", (uintmax_t)address, strerror(errno));
    return NULL;
  }
...
static void reg_write(volatile uint32_t *base, unsigned offset,
                      uint32_t value) {
...
static void dma_buffer_sync_for_cpu(const struct dma_buffer *buffer) {
  ...
  if (msync(buffer->mapping, buffer->mapping_size, MS_SYNC | MS_INVALIDATE) !=
          0 &&
      !warned) {
    fprintf(stderr, "WARNING: could not invalidate DMA buffer mapping: %s\n",
            strerror(errno));
    warned = 1;
  }
  __sync_synchronize();
...
static int ak09916_wait_aux_transaction(volatile uint32_t *spi) {
  ...
  while (monotonic_ms() < deadline) {
    ...
    if (icm_read_bank_register(spi, ICM_BANK_0, ICM_I2C_MST_STATUS, &status) !=
        0)
      return -1;
...
static int ak09916_write_register(volatile uint32_t *spi, uint8_t address,
  ...
  if (icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV4_ADDR,
                              AK09916_I2C_ADDR) != 0 ||
      icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV4_REG, address) !=
          0 ||
      icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV4_DO, value) != 0)
    return -1;
  if (icm_read_bank_register(spi, ICM_BANK_0, ICM_I2C_MST_STATUS, &status) != 0)
    return -1;
...
static int ak09916_read_register(volatile uint32_t *spi, uint8_t address,
  ...
  if (icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV4_ADDR,
      ...
      icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV4_REG, address) != 0)
    return -1;
  if (icm_read_bank_register(spi, ICM_BANK_0, ICM_I2C_MST_STATUS, &status) != 0)
    return -1;
  if (icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV4_CTRL,
...
static int initialize_ak09916(volatile uint32_t *spi) {
  ...
  if (icm_write_bank_register(spi, ICM_BANK_0, ICM_USER_CTRL,
      ...
      icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_MST_ODR_CONFIG, 0x00u) !=
          0 ||
      icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_MST_DELAY_CTRL, 0x00u) !=
          0)
    return -1;
  usleep(10000);
  ...
  if (wia1 != AK09916_WIA1_VALUE || wia2 != AK09916_WIA2_VALUE) {
    fprintf(stderr,
            "WARNING: AK09916 identity is 0x%02x/0x%02x, "
  ...
  if (ak09916_write_register(spi, AK09916_CNTL3, AK09916_RESET) != 0)
    return -1;
  usleep(100000);
  if (ak09916_write_register(spi, AK09916_CNTL2, AK09916_CONT_100HZ) != 0)
    return -1;

  if (icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV0_ADDR,
                              ICM_I2C_READ | AK09916_I2C_ADDR) != 0 ||
      icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV0_REG, AK09916_HXL) !=
          0 ||
      icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV0_CTRL,
                              ICM_I2C_SLV_EN | 6u) != 0 ||
      icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV1_ADDR,
                              ICM_I2C_READ | AK09916_I2C_ADDR) != 0 ||
      icm_write_bank_register(spi, ICM_BANK_3, ICM_I2C_SLV1_REG, AK09916_ST2) !=
          0 ||
...
static int initialize_icm20948(volatile uint32_t *core,
  ...
  if (identity != ICM_WHO_AM_I_VALUE) {
    fprintf(stderr,
            "WARNING: WHO_AM_I is 0x%02x, expected 0x%02x; "
            "continuing anyway.\n",
...
static int overlaps_system_ram(uint64_t address, size_t size) {
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    if (sscanf(line, " %llx-%llx : %63[^\n]", &first_ram, &last_ram, name) ==
            3 &&
        strcmp(name, "System RAM") == 0 && address <= (uint64_t)last_ram &&
        last >= (uint64_t)first_ram) {
      fclose(file);
      return 1;
    }
  }
...
static void print_sensor_bytes(const uint32_t sensor_words[5]) {
  ...
  printf("Magnetometer raw from AK09916 HXL..HZH: X=%" PRId16 " Y=%" PRId16
         " Z=%" PRId16 "\n",
...
static int run_axil_only_test(volatile uint32_t *core) {
  ...
  if (reg_read(core, CORE_COUNT) == 0) {
    fprintf(stderr,
            "FAIL: no sensor sample completed; status=0x%08" PRIx32
            ", error=0x%08" PRIx32 "\n",
  ...
  if (all_zero || all_ones) {
    fprintf(stderr,
            "FAIL: sensor returned all %s; check power, CS, MISO, "
            "and SPI mode.\n",
            all_zero ? "00" : "FF");
    return EXIT_FAILURE;
  }
  printf("PASS: response is not an all-zero or all-FF disconnected-bus "
         "pattern.\n");
  return EXIT_SUCCESS;
}
...
static int reset_configure_core_and_wait_intan(volatile uint32_t *core,
  ...
  fprintf(stderr,
          ...
          " error=0x%08" PRIx32 " missed_intan=%" PRIu32 " missed_icm=%" PRIu32
          "\n",
          reg_read(core, CORE_STATUS), reg_read(core, CORE_ERROR),
...
static int validate_intan_icm_packet(const volatile uint32_t *words,
  ...
  if (read_be32_volatile(trailer + 12u) != SENSOR_TEST_PACKET_TRAILER_BYTES ||
      read_be32_volatile(trailer + 16u) !=
    ...
    fprintf(stderr,
            "Invalid packet trailer: valid=%" PRIu32 " intan=%" PRIu32
            "/%" PRIu32 " icm=%" PRIu32 " icm_offset=%" PRIu32
            " trailer_offset=%" PRIu32 "\n",
...
static void print_intan_packet_channels(const volatile uint32_t *words) {
  ...
  for (frame_index = 0; frame_index < frame_count; ++frame_index) {
    uint32_t frame_offset =
        read_be32_volatile(trailer + 56u + 4u * frame_index);
    unsigned slot;
    ...
    for (slot = 0; slot < SENSOR_TEST_NUM_INTAN; ++slot) {
      ...
      for (channel = 0; channel < SENSOR_TEST_INTAN_CHANNELS; ++channel) {
        unsigned byte_index = (SENSOR_TEST_INTAN_CHANNELS - 1u - channel) * 2u;
        uint16_t sample =
            (uint16_t)(((uint16_t)measurement[1u + byte_index] << 8) |
...
static void print_dma_words(unsigned transfer_index, uint32_t irq_count,
                            uint32_t sample_count,
                            const volatile uint32_t *words, size_t word_count) {
  size_t i;
  ...
  for (i = 0; i < preview; ++i)
    printf("0x%08" PRIx32 "%c", words[i], i + 1 == preview ? '\n' : ' ');
  ...
  if (word_count > preview) {
    ...
    for (i = tail_start; i < word_count; ++i)
      printf("0x%08" PRIx32 "%c", words[i], i + 1 == word_count ? '\n' : ' ');
  }
  ...
  for (i = 0; i + 1 < word_count && magic_windows_printed < 4u; ++i) {
    if (words[i] == UINT32_MAX && words[i + 1] == UINT32_MAX) {
      ...
      printf("Words around magic candidate at byte 0x%zx:\n  ",
             i * sizeof(uint32_t));
      for (j = start; j < end; ++j)
        printf("0x%08" PRIx32 "%c", words[j], j + 1 == end ? '\n' : ' ');
...
static void print_dma_debug_words(uint32_t dmacr, uint32_t dmasr,
  ...
  printf("DMA debug: DMACR=0x%08" PRIx32 " DMASR=0x%08" PRIx32
         " DA=0x%08" PRIx32 " LENGTH=0x%08" PRIx32 "\n",
...
int sensor_test_prepare_dma_udmabuf_sized(sensor_test_t *test,
  ...
  if (open_udmabuf_sized(&test->buffer, device, required_bytes) != 0) {
    fprintf(stderr,
            "Cannot map %s, read its sysfs address, or find "
            "at least %zu DMA bytes.\n",
            device, required_bytes);
    fprintf(stderr,
            "Load u-dma-buf, or use --phys with kernel-reserved RAM.\n");
    return -1;
  }
  return 0;
}
...
int sensor_test_prepare_dma_reserved_sized(sensor_test_t *test,
  ...
  if (overlap > 0) {
    ...
    if (!is_redpitaya_reserved_memory(physical_address, required_bytes,
                                      &rp_start, &rp_size)) {
      fprintf(stderr,
              "Refusing DMA address 0x%08" PRIx64
    ...
    fprintf(stderr,
            "Using Red Pitaya reserved memory: start=0x%08" PRIx64
            ", size=0x%08" PRIx64 ".\n",
            rp_start, rp_size);
  }
  if (overlap < 0) {
  ...
  close_dma_buffer(&test->buffer);
  if (open_reserved_memory(&test->buffer, test->mem_fd, physical_address,
                           required_bytes) != 0) {
...
stop:
  ...
  if (!(reg_read(dma, S2MM_DMASR) & DMA_IDLE) ||
    ...
    fprintf(stderr,
            "Timed out: core status=0x%08" PRIx32 ", DMA status=0x%08" PRIx32
            "\n",
...
int sensor_test_run_dma_interrupts(sensor_test_t *test, const char *uio_device,
  ...
  while (transfer_count == 0 || completed < transfer_count) {
    ...
    if (!(dma_status & DMA_IOC_IRQ)) {
      fprintf(stderr,
              "UIO interrupt without DMA completion, "
    ...
    if (print_frames) {
      print_dma_frame(completed, irq_count, reg_read(core, CORE_COUNT), frame);
      fflush(stdout);
    }
...
int sensor_test_run_dma_interrupts_sized(sensor_test_t *test,
  ...
  if (dma_buffer_available_bytes(buffer) < transfer_bytes) {
    fprintf(stderr,
            "DMA buffer mapping is too small: need %zu bytes, "
            "have %zu bytes.\n",
